...
IndexedSuffixNode::Split(IndexedSuffixNode *old_child, size_t depth, const char *text,
                         size_t s, size_t e, size_t parent_tpl) {
  // ADS: previously, this function had a final instance variable
  // "index" just like in the ordinary SuffixTree class
  ...
  if (!has_child(i)) {
    ...
    if (depth < e - s) {
      if (depth < child[i]->edge_length()) {
        child[i] = Split(child[i], depth, text, s, e, path_length);
        return child[i];
      }
      else {
