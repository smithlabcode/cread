...
StadenPValue::normalize_matrix(ScoringMatrix& sm) const {
  ...
  auto scaler(std::bind(std::multiplies<float>(), _1, scale));
  for (sm_itr i = sm.begin(); i != sm.end(); ++i)
    std::transform(*i, *i + alphabet_size, *i, scaler);

  float min_entry = std::numeric_limits<float>::max();
  for (sm_itr i = sm.begin(); i != sm.end(); ++i)
    min_entry = std::min(min_entry, *std::min_element(*i, *i + alphabet_size));

  auto translater(std::bind(std::plus<float>(), _1, -min_entry));
  for (sm_itr i = sm.begin(); i != sm.end(); ++i)
    std::transform(*i, *i + alphabet_size, *i, translater);

  for (sm_itr i = sm.begin(); i != sm.end(); ++i)
    std::transform(*i, *i + alphabet_size, *i, nearbyint);
